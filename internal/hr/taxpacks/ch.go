...
var (
	...
	// (CHF 148,200 / yr). Above the ceiling, the employee's
	// *additional* contribution drops to 0% (the employer
	// Solidarity branch is not an employee deduction).
	//
	// The cap is enforced YTD-aware, mirroring the US pack's
	// OASDI pattern (us.go:124-130): each slip's ALV-base is
	// limited to `chALVAnnualCap - EmployeeInfo.YTDGross`, so once
	// the YTD AHV-base hits the annual ceiling no further ALV
	// accrues. This is the BSV-correct enforcement — the annual
	// ceiling is the only statutory cap; the often-cited CHF
	// 12,350 "monthly cap" is just the annual cap divided by 12
	// and applies only to regular-monthly payrolls in steady
	// state. A period-prorated cap (annualCap × days/365.25)
	// gives the same steady-state result for a regular cadence
	// but front-loads the ceiling for bonus slips and irregular
	// off-cycle payments. YTD-aware capping is correct for both
	// regular and irregular slips, and matches BSV's year-end
	// reconciliation logic.
	//
	// Callers must populate EmployeeInfo.YTDGross with the
	// employee's prior YTD AHV-base for the calendar year; the
	// payroll engine derives this from the year's prior slips
	// (same source the US pack consumes).
...
//   - CH_ALV (1.1% employee share with YTD-aware annual ceiling —
//     each slip's ALV-base is limited to `chALVAnnualCap -
//     YTDGross`, so once the YTD AHV-base reaches CHF 148,200 no
//     further ALV accrues. Mirrors the US pack's OASDI pattern.).
//
...
func (chPack) ComputeWithholding(_ context.Context, e EmployeeInfo, gross decimal.Decimal, period PayPeriod) ([]Deduction, error) {
	...
	// ALV — annual ceiling CHF 148,200 enforced YTD-aware. Once
	// the employee's year-to-date AHV-base reaches the ceiling,
	// no further ALV accrues on subsequent slips (the employer
	// then pays the Solidarity branch which is not an employee
	// deduction). Mirrors usPack's OASDI cap pattern. A slip that
	// straddles the ceiling is split: the portion that takes YTD
	// from below to above the cap is subject; the remainder is
