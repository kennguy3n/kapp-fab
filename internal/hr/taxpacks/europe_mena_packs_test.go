...
//                   annual ceiling CHF 148,200 enforced YTD-aware
//                   against EmployeeInfo.YTDGross)
//
...
// TestCHPackALVFirstSlipBelowAnnualCap pins the ALV behavior for
// a slip whose YTDGross + gross is still under the annual
// ceiling. The cap is enforced YTD-aware (against
// EmployeeInfo.YTDGross), so a high earner's first slip of the
// year at CHF 20,000 — even though it would breach the
// often-cited "CHF 12,350 / month" smoothing figure — is fully
// subject to ALV because the YTD allowance hasn't been exhausted.
// Subsequent slips will continue accruing ALV until YTD hits CHF
// 148,200, at which point ALV stops. See
// TestCHPackALVCapTriggeredByYTD for the cap-trigger transition.
//
//   AHV: 20,000 × 5.3% = 1,060.00 (no cap)
//   ALV: 20,000 × 1.1% = 220.00 (YTD=0, full slip subject)
func TestCHPackALVFirstSlipBelowAnnualCap(t *testing.T) {
	pack, _ := Lookup("CH")
	out, _ := pack.ComputeWithholding(context.Background(), EmployeeInfo{
		Resident: true, PermitType: "C", Canton: "ZH",
		YTDGross: decimal.Zero,
	...
	if alv := codes["CH_ALV"]; !alv.Equal(decimal.NewFromInt(220)) {
		t.Errorf("CH_ALV = %s; want 220.00 (20000 × 1.1%%, YTD=0 so full slip subject)", alv)
	}
}

// TestCHPackALVCapTriggeredByYTD pins the YTD-aware cap behavior
// across three transition points: (1) YTD well below cap — full
// slip subject; (2) YTD straddling the cap mid-slip — only the
// portion that takes YTD from below to above is subject; (3) YTD
// at or above cap — zero ALV. This mirrors usPack's OASDI cap
// pattern (us.go:127-130) and matches BSV's year-end
// reconciliation logic. A regression that re-introduces
// period-prorated capping would fail the "straddle" case below.
func TestCHPackALVCapTriggeredByYTD(t *testing.T) {
	pack, _ := Lookup("CH")

	t.Run("ytd well below cap — full slip subject", func(t *testing.T) {
		// YTDGross = 50,000; gross = 7,000; YTD after = 57,000.
		// Still well under 148,200, so the entire 7,000 is
		// subject: ALV = 7,000 × 1.1% = 77.00.
		out, _ := pack.ComputeWithholding(context.Background(), EmployeeInfo{
			Resident: true, PermitType: "C", Canton: "ZH",
			YTDGross: decimal.NewFromInt(50000),
		}, decimal.NewFromInt(7000), PayPeriod{
			Start: time.Date(2026, 1, 1, 0, 0, 0, 0, time.UTC),
			End:   time.Date(2026, 1, 14, 0, 0, 0, 0, time.UTC),
		})
		codes := indexByCode(out)
		if alv := codes["CH_ALV"]; !alv.Equal(decimal.NewFromInt(77)) {
			t.Errorf("CH_ALV = %s; want 77.00 (YTD=50k, full 7,000 × 1.1%% subject)", alv)
		}
	})

	t.Run("ytd straddles cap mid-slip — only sub-cap portion subject", func(t *testing.T) {
		// YTDGross = 145,000; gross = 10,000; YTD after = 155,000.
		// Cap is 148,200 so only 148,200 - 145,000 = 3,200 of
		// the slip is subject. ALV = 3,200 × 1.1% = 35.20.
		// (A period-prorated cap would instead cap at the slip's
		// proration ~ 12,578 × 1.1% ≈ 138.36, which is wrong
		// because it ignores that YTD already consumed most of
		// the annual allowance.)
		out, _ := pack.ComputeWithholding(context.Background(), EmployeeInfo{
			Resident: true, PermitType: "C", Canton: "ZH",
			YTDGross: decimal.NewFromInt(145000),
		}, decimal.NewFromInt(10000), monthPeriod())
		codes := indexByCode(out)
		if alv := codes["CH_ALV"]; !alv.Equal(decimal.NewFromFloat(35.20)) {
			t.Errorf("CH_ALV = %s; want 35.20 (only 3,200 sub-cap portion × 1.1%%); "+
				"a period-prorated cap would incorrectly emit ~138.36", alv)
		}
	})

	t.Run("ytd above cap — no ALV", func(t *testing.T) {
		// YTDGross = 150,000 > cap 148,200; whole slip exempt.
		out, _ := pack.ComputeWithholding(context.Background(), EmployeeInfo{
			Resident: true, PermitType: "C", Canton: "ZH",
			YTDGross: decimal.NewFromInt(150000),
		}, decimal.NewFromInt(5000), monthPeriod())
		codes := indexByCode(out)
		if _, ok := codes["CH_ALV"]; ok {
			t.Errorf("CH_ALV should not be emitted once YTD ≥ cap; codes=%+v", codes)
		}
	})

	t.Run("bonus slip on top of regular YTD — no double-cap", func(t *testing.T) {
		// The scenario the Devin Review comment flagged:
		// employee already at CHF 130,000 YTD from regular pay,
		// receives a separate bonus slip of CHF 20,000. The cap
		// must recognise that only 18,200 of the bonus is subject
		// to ALV (148,200 - 130,000), so ALV = 18,200 × 1.1%
		// = 200.20. A period-prorated cap would treat this slip
		// independently and cap at the monthly proration
		// (~12,578), yielding 138.36 — over-collecting against
		// the annual ceiling.
		out, _ := pack.ComputeWithholding(context.Background(), EmployeeInfo{
			Resident: true, PermitType: "C", Canton: "ZH",
			YTDGross: decimal.NewFromInt(130000),
		}, decimal.NewFromInt(20000), monthPeriod())
		codes := indexByCode(out)
		if alv := codes["CH_ALV"]; !alv.Equal(decimal.NewFromFloat(200.20)) {
			t.Errorf("bonus-slip CH_ALV = %s; want 200.20 (18,200 sub-cap × 1.1%%); "+
				"period-prorated capping would incorrectly emit ~138.36 and double-count "+
				"the annual ceiling across the regular + bonus slips", alv)
		}
	})
}
...
// bracket (income > CHF 755,200 / year) on a high-earner first
// slip of the year. With YTDGross=0 the ALV cap is not yet
// triggered, so ALV applies to the full slip. A resident
// B-permit holder in Geneva at CHF 100,000 / month:
...
//   ALV (YTD=0)   = 100,000 × 0.011 = 1,100.00 (cap not yet hit)
// The cap-trigger transition (subsequent slips that take YTD
// across the annual ceiling) is covered by
// TestCHPackALVCapTriggeredByYTD above.
func TestCHPackTopBracketHighEarner(t *testing.T) {
	...
	if alv := codes["CH_ALV"]; !alv.Equal(decimal.NewFromInt(1100)) {
		t.Errorf("CH_ALV = %s; want 1,100.00 (YTD=0, full slip 100,000 × 1.1%% — cap not yet hit)", alv)
	}
}
