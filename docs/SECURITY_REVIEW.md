...
| `KAPP_REQUIRE_JWT` | Default **true** in any non-development `KAPP_ENV`. A non-dev boot without a working JWT signer is refused at boot time across `services/api`, `services/importer`, and `services/agent-tools` — all three gate on `cfg.RequireJWT && cfg.IsNonDev()`, so a prod/staging deploy without a signer fails loudly rather than booting into a 100%-403 state (`platform.TenantMiddleware` rejects every request in production). In development the signer stays optional and the legacy `X-Tenant-ID` header path remains available; set `KAPP_REQUIRE_JWT=0` to opt a non-dev deploy back into that bridge mode. | Critical in prod |
...
| `KAPP_CSRF_COOKIE_SECURE` | Controls the `Secure` flag on the issued CSRF cookie. **Secure-by-default outside development (Workstream 6):** when unset it now follows the deployment posture — `true` in any non-development `KAPP_ENV` (`staging` / `production`, all served over HTTPS, so the cookie must not be replayable over plain HTTP), `false` only for development-class envs so local-dev HTTP keeps working. This is gated on the same non-dev predicate as `KAPP_REQUIRE_REDIS` / `KAPP_REQUIRE_JWT`, so staging is hardened too. An explicit value always wins, including an explicit `false` outside development (operator override for an HTTP-only environment); that override is **not fatal** but is surfaced as a startup `WARN` (see `Config.Warnings`) because it ships a CSRF cookie a browser may send over cleartext. | Medium outside dev |
