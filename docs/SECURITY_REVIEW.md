...
| `KAPP_CSRF_COOKIE_SECURE` | Controls the `Secure` flag on the issued CSRF cookie. **Production-safe default (Workstream 6):** when unset it now follows the deployment posture — `true` when `KAPP_ENV=production`/`prod` (the cookie is served over HTTPS and must not be replayable over plain HTTP), `false` otherwise so local-dev HTTP keeps working. An explicit value always wins, including an explicit `false` in production (operator override); that override is **not fatal** but is surfaced as a startup `WARN` (see `Config.Warnings`) because it ships a CSRF cookie a browser may send over cleartext. | Medium in prod |
