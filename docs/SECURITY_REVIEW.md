...
| `KAPP_PLATFORM_ADMIN_USERS` | Bootstrap-time UUID list, **two-step by design**. (1) the candidate user signs in via SSO — this creates the `users` row but the row is _not_ promoted because the DB-generated UUID was unknown when the env var was set. (2) operator reads the new UUID (`SELECT id FROM users WHERE email = ...`) and appends it to the env var. (3) the user's next **fresh SSO exchange** (NOT a refresh of an existing session) sees the match and persists `is_platform_admin = TRUE`. Promotion happens inside `upsertUser`, which only runs in `Exchange`; the `Refresh` path re-queries the DB but the row is still `is_platform_admin = FALSE` at that point, so the refreshed token would NOT carry the admin claim. In practice the candidate user must log out and log back in (or have their existing session revoked) so step (3) goes through `Exchange`. Re-read on every fresh exchange so operators can keep appending admin UUIDs without restarting. Unset is fine once at least one row in `users` already has `is_platform_admin = TRUE`. | Low (bootstrap-only) |
