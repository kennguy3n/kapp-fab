...
Full historical record of phase-by-phase deliverables, the
"Cross-cutting" platform-primitive inventory, and the First-Coding-
Slice acceptance checklist for kapp-fab. For the current high-level
...
- [x] Advanced accounting consolidation — operator-scoped, admin-only cross-tenant rollup driven by `consolidation_groups` + `consolidation_runs` (PR #56, `migrations/000030_consolidation.sql`; `internal/ledger/consolidation.go`; admin-only `POST /api/v1/admin/consolidation/runs`)
