...
> **Last Updated:** 2026-04-25 (ZK Object Fabric integration for per-tenant zero-knowledge encrypted file storage; per-tenant feature flags wired through `FeatureFromPath` for `/records/{ktype}` routes; `GET /api/v1/tenants/me/features` + `/me/usage` + `/me/usage/history` + `/me/plan` JWT-resolved endpoints; Redis-backed distributed rate limiter Lua refill math fixed and miniredis test suite added; inventory reorder automation integration test added; daily tenant usage snapshot worker for storage_bytes + krecord_count; usage history bar chart on the admin dashboard.)
>
...
**Phase I — Multi-currency, Helpdesk, Reporting, Dashboard + Phase J platform slice**
**Status:** Complete (Phase I four deliverables merged via PR #24: multi-currency store + KType + handlers; helpdesk KTypes + SLA store + agent tools + `/ticket` command; report builder grammar + saved reports + runner; KPI dashboard summary endpoint + landing page). Phase H hardening slice remains Complete from PR #22. Phase J feature slice complete: scheduled actions framework (PR #27), credit-limit enforcement (PR #28), helpdesk SLA breach sweeper (PR #29), recurring invoice generator (PR #30), payment terms templates (PR #31), full payroll engine with idempotent PostPayRun (PRs #32/#33/#34), per-tenant feature flags + inventory reorder automation + Redis-backed distributed rate limiting + tenant resource metering (PR #35). Next focus: bulk actions on RecordListPage, full-text search across KTypes, webhook management with delivery log, print/PDF generation, helpdesk customer portal.
