...
func buildDeps(ctx context.Context, cfg *platform.Config) (deps *apiDeps, cleanup func(), err error) {
	...
	// Phase N9b — procurement.purchase_requisition state machine
	// surfaced through the agent tool layer. The poster is shared
	// with the HTTP handler so a tool-driven approve/convert and
	// an HTTP-driven approve/convert post against the same state
	// machine and idempotency guards.
	requisitionPoster := sales.NewRequisitionPoster(recordStore)
	agents.RegisterRequisitionTools(executor, recordStore, requisitionPoster)
	...
	d := &apiDeps{
		...
		requisitionPoster:      requisitionPoster,
		authzEval:              authzEval,
		auditor:                auditor,
