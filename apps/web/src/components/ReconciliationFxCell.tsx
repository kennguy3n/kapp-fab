 ...
 * base-currency equivalent plus the FX difference (base − face value). If
 * no rate is published it says so explicitly rather than inventing a
 * figure — the operator must not be shown a fabricated base amount. This
 * is the surface that stops a foreign line from being silently matched to
 * a base-currency ledger entry on a bare numeric coincidence.
 */
    ...
    <span className={`flex flex-col ${align === "right" ? "items-end" : "items-start"}`}>
      ...
      {conv ? (
        ...
        >
          ...
          {" · "}
          {rt("reconciliation.fx.diff")}:{" "}
          {formatAmount(conv.base - amount, baseCurrency)}
        </span>
      ) : (
